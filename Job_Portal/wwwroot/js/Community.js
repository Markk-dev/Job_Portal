document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#commentModal .close-button').addEventListener('click', function() {
        closeCommentModal();
        });
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            posts.forEach(post => {
                ...
                const authorName = post.querySelector('.post-author').textContent.toLowerCase();
                post.style.display = (postContent.includes(searchTerm) || authorName.includes(searchTerm)) ? '' : 'none';
            });
        });
        }
    ...
    button.innerHTML = `<i class="material-icons">chat_bubble_outline</i> ${count ? count[0] : '0'}`;
        });
